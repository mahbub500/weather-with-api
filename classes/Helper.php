...
class Helper {
	...
	public static function get_posts( $args = [], $show_heading = false, $show_cached = false ) {
		...
		// use cache
		if( true === $show_cached && ( $cached_posts = wp_cache_get( "weatherapit_{$_args['post_type']}", 'weatherapit' ) ) ) {
			$posts = $cached_posts;
		}
		...
		// don't use cache
			...
			wp_cache_add( "weatherapit_{$_args['post_type']}", $posts, 'weatherapit', 3600 );
		}

		$posts = $show_heading ? [ '' => sprintf( __( '- Choose a %s -', 'weatherapit' ), $_args['post_type'] ) ] + $posts : $posts;

		return apply_filters( 'weatherapit_get_posts', $posts, $_args );
	}
	 ...
	 *
	 * It'll look into /weather-with-api directory of your active theme
	 * first. if not found, default template will be used.
	 * can be overwriten with weather-with-api_template_overwrite_dir hook
	 *
	...
	public static function get_template( $slug, $base = 'views', $args = null ) {
		...
		$overwrite_template_dir = apply_filters( 'weatherapit_template_overwrite_dir', get_stylesheet_directory() . '/weather-with-api/', $slug, $base, $args );
		
		// default template directory
		$plugin_template_dir = dirname( Weather_With_Api ) . "/{$base}/";
		...
		elseif ( file_exists( $plugin_template_path ) ) {
			...
			return __( 'Template not found!', 'weather-with-api' );
		}
	}
}